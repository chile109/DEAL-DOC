...
The DEAL system’s Basic FSM is composed of four main Scriptable Objects (SO): **State Machine**, **State**, **Action**, and **Condition**. Each of these components can be fully customized to fit your game's needs.
...
2. **Condition (ScriptableCondition SO)**: This checks whether the transition should occur (required).
...
This is the core structure representing a state within the FSM, responsible for executing every action listed in the target `ScriptableAction[]` array. Each `ScriptableState` can trigger different actions at various points:
